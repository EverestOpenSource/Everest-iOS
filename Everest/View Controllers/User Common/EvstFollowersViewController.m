...
@interface EvstFollowersViewController ()
@property (nonatomic, strong) EvstUserTableView *tableView;
@end

@implementation EvstFollowersViewController
...
- (void)viewDidLoad {
  ...
  self.navigationItem.title = self.navigationItem.accessibilityLabel = kLocaleFollowers;
  [self setupTableView];
}
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  [EvstAnalytics trackView:kEvstAnalyticsDidViewFollowersList objectUUID:self.user.uuid];
}
...
- (void)setupTableView {
  ...
  self.tableView.accessibilityLabel = kLocaleFollowersTable;
...
- (void)tableView:(EvstUserTableView *)tableView getUsersForPage:(NSUInteger)page {
  [EvstFollowsEndPoint getFollowersForUser:self.user page:self.tableView.currentPage success:^(NSArray *users) {
